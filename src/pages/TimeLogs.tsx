...
import {
  usePendingExcursions,
  useUpdateExcursionReason,
} from '../util/queries/excursions';
...
export default function TimeLogs() {
  ...
  const { mutateAsync: updateExcursionReason, isPending: isUpdatingReason } =
    useUpdateExcursionReason();
  const [reasonModalVisible, setReasonModalVisible] = useState(false);
  const [selectedExcursionId, setSelectedExcursionId] = useState<number | null>(
    null,
  );
  const [confirmBreakAction, setConfirmBreakAction] = useState<
  ...
  let status: any = serverStatus;
  if (!isOnline && status && offlineQueue.length > 0) {
    status = JSON.parse(JSON.stringify(serverStatus)); // deep clone; serverStatus already confirmed truthy above, so this can't be null
    const lastPunch = offlineQueue[offlineQueue.length - 1];
    if (lastPunch.action === 'time_in') {
      status.open_log = {
        time_in: lastPunch.timestamp,
        break_out: null,
        break_in: null,
        time_out: null,
      };
  ...
  const isRemoved = status?.placement_status === 'removed';
  // Drives whether the "Update Task Note & Photos" button/reminder shows at all.
  // Only checked while there's an open log — a closed day has nothing to nudge.
  const openLogId: number | null = status?.open_log?.id ?? null;
  const { data: taskChecker } = useTaskChecker(openLogId);
  ...
  useEffect(() => {
    ...
    Boolean(status?.open_log), // was: status?.open_log (new object every clone)
    dashboard?.company?.latitude,
    dashboard?.company?.longitude,
    dashboard?.company?.radius_meters,
  ]);
  ...
  const punchConfig: Record<PunchPhase, any> = {
    ...
    break_out: {
      ...
      hint: isSyncing
        ? 'Syncing...'
        : !isInGeofence
          ? 'Fieldwork mode (Break allowed)'
          : 'Tap to break out',
      onPress: () => handleActionPress('break_out'),
      color: '#F59E0B',
    },
    break_in: {
      label: 'Break In',
      ...
      hint: isSyncing
        ? 'Syncing...'
        : !isInGeofence
          ? 'Fieldwork mode (Resume allowed)'
          : 'Tap to resume work',
      onPress: () => handleActionPress('break_in'),
      color: '#10B981',
    },
    punch_out: {
      label: 'Punch Out',
      ...
      hint: isSyncing
        ? 'Syncing...'
        : !isInGeofence
          ? 'Fieldwork mode (Note required)'
          : 'Tap to punch out',
      onPress: () => handleActionPress('punch_out'),
      color: '#EF4444',
    },
    done: {
      label: 'Day Complete 🎉',
  ...
  // Task note/photo reminder: only meaningful while there's an open log to attach to.
  const taskNeedsUpdate =
    Boolean(status?.open_log?.id) && Boolean(taskChecker?.needs_update);
        ...
        <View className="px-5" style={{ marginTop: -28 }}>
          {pendingExcursions && pendingExcursions.length > 0 && (
            <Animated.View
              ...
              className="flex-row items-center rounded-2xl bg-amber-50 px-4 py-3 mb-4 border border-amber-100"
            >
              <AlertCircle color="#D97706" size={18} />
              <View className="ml-2.5 flex-1">
                <Text className="text-[12px] font-medium text-amber-700">
                  You have {pendingExcursions.length} pending excursion(s).
                </Text>
                <Pressable
                  onPress={() => {
                    setSelectedExcursionId(pendingExcursions[0].id);
                    setReasonModalVisible(true);
                  }}
                >
                  <Text className="text-[12px] font-bold text-amber-700 mt-1 underline">
                    Provide reason
                  </Text>
                </Pressable>
              </View>
          ...
          {locationError && (
            <Animated.View
              entering={FadeInUp.duration(400)}
              className="flex-row items-center rounded-2xl bg-red-50 px-4 py-3 mb-4 border border-red-100"
            >
              <AlertCircle color="#DC2626" size={18} />
              <Text className="ml-2.5 text-[12px] font-medium text-red-700 flex-1">
                {locationError}
              </Text>
            </Animated.View>
          )}
    ...
    <PunchActionCard punch={punch} isPunching={isPunching} />
          ...
          {taskNeedsUpdate && (
            ...
            >
              ...
              <View className="ml-2.5 flex-1">
                <Text className="text-[12px] font-medium text-amber-700">
                  {taskChecker?.message ??
                    "Don't forget to log your task note and photo(s) for today."}
                </Text>
                <Pressable
                  onPress={() =>
          ...
          >
            <View className="px-5 pt-5 pb-2 flex-row items-center justify-between">
              ...
              <StatusBadge
                label={status.today_attendance.label}
            ...
            <View className="px-5 pb-5">
              <InfoRow
                icon={Clock}
                label="Schedule"
                value={status.today_attendance.schedule_label}
                themeColor={themeColor}
              />
              <InfoRow
                icon={Clock}
                label="Hours Today"
                value={`${status.today_hours} hrs (${status.today_minutes} min)`}
                themeColor={themeColor}
              />
            </View>
          </Animated.View>
          ...
          >
            ...
            <View className="px-5 pb-5">
              <InfoRow
                icon={LogIn}
                label="Time In"
                value={formatLogTime(timelineEntry?.time_in)}
                themeColor={themeColor}
              />
              <InfoRow
                icon={Coffee}
                label="Break Out"
                value={formatLogTime(timelineEntry?.break_out)}
                themeColor={themeColor}
              />
              <InfoRow
                icon={Sunrise}
                label="Break In"
                value={formatLogTime(timelineEntry?.break_in)}
                themeColor={themeColor}
              />
              <InfoRow
                icon={LogOut}
                label="Time Out"
                value={formatLogTime(timelineEntry?.time_out)}
                themeColor={themeColor}
              />

              {status?.open_log?.id && taskNeedsUpdate && (
                <Pressable
                  onPress={() =>
                    navigation.navigate('UpdateTask', {
                      timeLogId: status.open_log.id,
              ...
              {status?.open_log?.id &&
                taskChecker &&
                !taskChecker.needs_update && (
                  <View className="mt-4 flex-row items-center justify-center rounded-2xl py-3 bg-green-50">
                    <CheckCircle2 color="#16A34A" size={16} />
                    <Text className="ml-2 font-semibold text-[13px] text-green-700">
                      Task note & photos logged
                    </Text>
                  </View>
                )}
            </View>
          ...
          >
            <View className="px-5 pt-5 pb-2 flex-row items-center justify-between">
              ...
              <View
                className={`rounded-full px-3 py-1 ${status?.geofence?.configured ? 'bg-green-50' : 'bg-red-50'
                  }`}
              >
                <Text
                  className={`text-[11px] font-bold uppercase tracking-wide ${status?.geofence?.configured
                    ? 'text-green-700'
                    : 'text-red-600'
                    }`}
                >
            ...
            <View className="px-5 pb-4 pt-2">
              <InfoRow
                icon={MapPin}
                label="Company"
                value={status?.geofence?.company_name || 'Unassigned'}
                themeColor={themeColor}
              />
              <InfoRow
                icon={MapPin}
                label="Radius"
                value={
                  status?.geofence?.radius_meters
                    ? `${status.geofence.radius_meters}m`
                    : 'N/A'
                }
                themeColor={themeColor}
              />
